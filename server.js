...
app.post('/api/reports/restore', async (req, res) => {
  try {
    ...
    try {
      ...
      if (d.members?.length)       for (const r of d.members)       await conn.query('INSERT IGNORE INTO members (id,name,phone,cls,shift,plan,category,`from`,`to`,feeStatus,seat,color,addr,guardian,gphone,aadhar,aadharImg,photo) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)', [r.id,r.name,r.phone,r.cls,r.shift,r.plan,r.category,r.from,r.to,r.feeStatus,r.seat,r.color,r.addr,r.guardian,r.gphone,r.aadhar,r.aadharImg,r.photo]);
      if (d.feeRecords?.length)    for (const r of d.feeRecords)    await conn.query('INSERT IGNORE INTO fee_records (id,memberId,memberName,plan,shift,category,amount,paidAmount,dueAmount,date,month,mode,notes,status) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?)', [r.id,r.memberId,r.memberName,r.plan,r.shift,r.category,r.amount,r.paidAmount,r.dueAmount,r.date,r.month,r.mode,r.notes,r.status]);
      if (d.expenses?.length)      for (const r of d.expenses)      await conn.query('INSERT IGNORE INTO expenses (id,cat,`desc`,amount,date,mode,notes) VALUES (?,?,?,?,?,?,?)', [r.id,r.cat,r.desc,r.amount,r.date,r.mode,r.notes]);
      if (d.employees?.length)     for (const r of d.employees)     await conn.query('INSERT IGNORE INTO employees (id,name,role,phone,salary,join_date,addr) VALUES (?,?,?,?,?,?,?)', [r.id,r.name,r.role,r.phone,r.salary,r.join_date,r.addr]);
      if (d.notices?.length)       for (const r of d.notices)       await conn.query('INSERT IGNORE INTO notices (id,title,type,body,date) VALUES (?,?,?,?,?)', [r.id,r.title,r.type,r.body,r.date]);
      if (d.salaryRecords?.length) for (const r of d.salaryRecords) await conn.query('INSERT IGNORE INTO salary_records (id,empId,empName,month,amount,date,mode,notes) VALUES (?,?,?,?,?,?,?,?)', [r.id,r.empId,r.empName,r.month,r.amount,r.date,r.mode,r.notes]);
      if (d.attendance?.length)    for (const r of d.attendance)    await conn.query('INSERT IGNORE INTO attendance (memberId,memberName,date,shift,seat,`in`,`out`,present) VALUES (?,?,?,?,?,?,?,?)', [r.memberId,r.memberName,r.date,r.shift,r.seat,r.in,r.out,r.present?1:0]);
