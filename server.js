  ...
  try {
    ...
    if (d._clearOnly) {
      ...
      try {
        ...
        await conn.commit();
        console.log('🗑️ Tables cleared (clearOnly)');
        return ok(res, { done: true });
      } catch(e) { await conn.rollback(); throw e; }
      finally { conn.release(); }
    }

    // ── CHUNK MODE — sirf jo bheja hai woh INSERT karo ──────────
    if (d._chunkMode) {
      const conn = await pool.getConnection();
      await conn.beginTransaction();
      try {
    ...
    // ── FULL RESTORE (legacy / fallback) ────────────────────────
    console.log('🔄 Full restore — members:', d.members?.length||0, 'fees:', d.feeRecords?.length||0, 'attendance:', d.attendance?.length||0);
    const conn = await pool.getConnection();
    await conn.beginTransaction();
    try {
      for (const t of ['members','fee_records','attendance','notices','expenses','employees','salary_records','fee_structure'])
