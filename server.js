...
app.post('/api/reports/restore', async (req, res) => {
  try {
    ...
    try {
      ...
      // ── members: backup uses camelCase keys, DB uses snake_case columns ──
      if (d.members?.length) {
        for (const r of d.members) {
          await conn.query(
            `INSERT IGNORE INTO members
              (id, name, phone, cls, shift, plan, seat, color, addr,
               guardian, gphone, aadhar, aadhar_img, photo,
               fee_status, join_date, valid_till)
             VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)`,
            [
              ...
              r.aadhar || null,
              r.aadhar_img || r.aadharImg || null,
              r.photo || null,
              r.fee_status || r.feeStatus || 'Due',
              r.join_date || r.from || null,
              r.valid_till || r.to || null
            ]
          );
        }
      }

      // ── fee_records: backup uses camelCase, DB uses snake_case ──
      if (d.feeRecords?.length) {
        for (const r of d.feeRecords) {
          await conn.query(
            `INSERT IGNORE INTO fee_records
              (id, member_id, member_name, plan, shift, amount,
               paid_amount, due_amount, date, month, mode, status, notes)
             VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?)`,
            [
              r.id,
              r.member_id || r.memberId || null,
              r.member_name || r.memberName || null,
              r.plan || null, r.shift || null,
              r.amount || 0,
              r.paid_amount || r.paidAmount || r.amount || 0,
              r.due_amount || r.dueAmount || 0,
      ...
      // ── expenses: backup may use 'desc', DB uses 'description' ──
      if (d.expenses?.length) {
        for (const r of d.expenses) {
          await conn.query(
            `INSERT IGNORE INTO expenses (id, cat, description, amount, date, mode, notes)
             VALUES (?,?,?,?,?,?,?)`,
            [
              r.id, r.cat || 'other',
      ...
      // ── salary_records: backup uses empId/empName, DB uses emp_id/emp_name ──
      if (d.salaryRecords?.length) {
        for (const r of d.salaryRecords) {
          await conn.query(
            `INSERT IGNORE INTO salary_records
              (id, emp_id, emp_name, month, amount, date, mode, notes)
             VALUES (?,?,?,?,?,?,?,?)`,
            [
              r.id,
              r.emp_id || r.empId || null,
              r.emp_name || r.empName || null,
      ...
      // ── attendance: backup uses memberId/in/out, DB uses member_id/check_in/check_out ──
      if (d.attendance?.length) {
        for (const r of d.attendance) {
          await conn.query(
            `INSERT IGNORE INTO attendance
              (date, member_id, member_name, shift, seat, check_in, check_out, present)
             VALUES (?,?,?,?,?,?,?,?)`,
            [
              r.date,
              r.member_id || r.memberId || null,
              ...
              r.shift || null,
              r.seat || null,
              r.check_in || r.in || null,
              r.check_out || r.out || null,
              r.present ? 1 : 0
            ]
          );
        }
      ...
      // ── fee_structure: backup uses plan/shift, DB has plan_name/shift ──
      if (d.feeStructure?.length) {
        for (const r of d.feeStructure) {
          await conn.query(
            `INSERT INTO fee_structure (plan_name, shift, amount) VALUES (?,?,?)
